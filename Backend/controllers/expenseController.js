...
module.exports.multiCreateExpenses = async (req, res) => {
  try {
    // console.log("Raw expenses data:", req.body.expenses);
    let expensesArray;
...
module.exports.DueDateReminderExpense = async (req, res) => {
  ...
  try {
    ...
    const dueExpenses = userExpenses.filter((expense) => {
      ...
      dueDateObj.setHours(0, 0, 0, 0); // Ensure due date has no time component
      const today = new Date();
      ...
      for (const daysBefore of i) {
        ...
        if (today.getTime() === reminderDate.getTime()) {
          return true; // Return true if the reminder matches today
        }
      }
      return false; // If no match is found, exclude the expense
    });
...
const upload = multer({ storage: storage }).array("csvFiles", 10);
  ...
  upload(req, res, async (err) => {
    ...
    try {
      ...
      for (const record of results) {
        ...
        const parsedWarrentyDate = parse(record.warrentyDate, "dd-MM-yyyy", new Date());

        if (isNaN(parsedDate) || isNaN(parsedWarrentyDate)) {
        ...
        // Create a report for the new expense
        const reportData = {
          Title: record.date,
          Category: record.category,
